...
\begin{sectionbox}
    ...
    $[v]_{B_{neu}}=(a_1,...,a_n)^T$ mit $[v]_{B_{alt}}\overset{!}{=}a_1[b_{1,neu}]_{B_{alt}} + ... + a_n[b_{n,neu}]_{B_{alt}}$\\
...
\begin{sectionbox}
    ...
    $D_{\text{Basis im Definitionsraum, Basis im Bildraum}}$\\
    ...
    $D_{B_{neu}, B}=\begin{bmatrix}
            [f(b_{1, neu})]_B, ..., [f(b_{n,neu})]_B
        \end{bmatrix}$\\

    Basiswechsel im Bildraum mit neuer Basis $B_{neu}$:\\
    $D_{B, B_{neu}}=P_{B_{neu} \leftarrow B_{alt}} \times D_{B, B_{alt}}$\\
    $D_{B, B_{neu}}=\begin{bmatrix}
            [f(b_1)]_{B_{neu}}, ..., [f(b_n)]_{B_{neu}}
        \end{bmatrix}$\\

    Basiswechsel im Definitionsraum mit $B_{neu}$ und Bildraum mit $C_{neu}$:\\
    $D_{B_{neu}, C_{neu}}=P_{C_{neu} \leftarrow C_{alt}} \times D_{B_{alt},C_{alt}} \times P_{B_{alt} \leftarrow B_{neu}}$\\
    $D_{B_{neu}, C_{neu}}=\begin{bmatrix}
            [f(b_{1, neu})]_{C_{neu}},...,[f(b_{n, neu})]_{C_{neu}} \\
        \end{bmatrix}$\\

    \subsubsection{Spezialfälle}
    \textbf{Basiswechsel im Definitionsraum mit Standard-Basis im Bildraum:}\\
    Bilder der neuen Basisvektoren sind die Spalten der Darstellungsmatrix\\

    \textbf{Basiswechsel im Bildraum mit Standard-Basis im Definitionsraum:}\\
    Drücke die Bilder der Standard-Basisvektoren in der neuen Basis $\{b_1, ..., b_n\}$ aus:
    $f(e_j)\overset{!}{=}a_1b_1 + ... + a_nb_n$.
    Die Koeffizienten $a_1,...,a_n$ bilden die Spalten der Darstellungsmatrix.
\end{sectionbox}
...
\begin{sectionbox}
    ...
    Abbildung $\pi_U : V \rightarrow U$ die Vektoren in $V$ auf einen Unterraum projeziert.\\
    ...
    \begin{itemize}
        ...
        \item $U^\perp$ sind alle Vektoren, dessen Skalarprodukt mit Vektoren aus dem Unterraum $0$ ist: $A^Tv=0$ mit $A$ als Matrix der Basen des Unterraums als Spalten.
    \end{itemize}

    \subsubsection{Adjungierte $f^*$ von $f \in \End(V)$}
    $\langle f(v), w \rangle = \langle v, f^*(w) \rangle $\\
    Ist $B$ Orthonormalbasis von $V$, so gilt: $D_{B,B}(f^*)=D_{B,B}(f)^T$

    \subsubsection{Orthogonale Abbildung}
    Bildet Orthonormalbasis auf Orthonormalbasis ab $\rightarrow$ Zeilen und Spalten der orthogonalen Matrix $A$ sind Orthonormalbasen.\\
    $A^T=A^{-1}$ und $\det A = \pm 1$
\end{sectionbox}
...
\begin{sectionbox}
    ...
    \begin{tabular}{c|c}
        ...
        \begin{pmatrix}
            ...
            \end{pmatrix}$ &
        $\begin{matrix}
                 || v \times w || = ||v|| \: ||w|| \sin \angle (v,w) \\
                 \det (u, v, w) = \langle u, v \times w \rangle
             \end{matrix}$
    \end{tabular}
    \normalsize
\end{sectionbox}
...
\begin{sectionbox}
    Berechnung zu Matrix $A$:\\
    \textbf{Eigenwerte}: $\chi_A(\lambda):=\det(A-\lambda I)\overset{!}{=}0$\\
    \textbf{Eigenvektoren}: $E_\lambda=\Kern(A-\lambda I)$
\end{sectionbox}

\begin{sectionbox}
    \subsubsection{Definitionen}
    \begin{itemize}
        \item \textbf{Spektrum}: Menge an Eigenwerten einer Matrix
        \item \textbf{Eigenraum $E_\lambda$ zum Eigenwert $\lambda$}: Unterraum, den die Eigenvektoren zu einem Eigenwert $\lambda$ aufspannen
        \item \textbf{geometrische Vielfachheit}: $\dim E_\lambda$
        \item \textbf{algebraische Vielfachheit}: Die Vielfachheit der Nullstelle des charakteristischen Polynoms $\chi_A$ (z.B. doppelte Nullstelle)
    \end{itemize}
\end{sectionbox}
...
\begin{sectionbox}
    \subsubsection{Eigenschaften}
    \begin{itemize}
        \item Basiswechsel mit Basiswechselmatrix $B$: $B^{-1}AB$ ändert die Eigenwerte von $A$ nicht
        \item Eigenvektoren zu verschiedenen Eigenwerten sind linear unabhängig
    \end{itemize}
\end{sectionbox}

\subsection{unitäre Vektorräume}
Vektorräume über $\mathbb C$, wobei gilt
\begin{itemize}
    \item $\langle \cdot, \cdot \rangle$ ist linear im ersten Argument (2. Argument konstant gehalten)
    \item $\langle v,w_1+w_2\rangle = \langle v,w_1\rangle + \langle v,w_2 \rangle$ (nur im 2. Argument)
    \item $\langle v, \lambda w\rangle = \overline \lambda \langle v,w \rangle$ (nur im 2. Argument)
    \item $\langle v,w \rangle = \overline{\langle w,v \rangle}$
    \item $\langle v,v \rangle > 0 \ \forall v \in V\setminus \{0\}$
\end{itemize}
\begin{sectionbox}
    Äquivalente (\equival) Aussagen:
    \begin{itemize}
        \item $f$ ist unitär
        \item $f$ ist längentreu
        \item $f^* \circ f=f \circ f^*=id \equival A^*A=AA^*=I$
        \item Die Zeilen und Spalten von $A$ bilden jeweils eine Orthonormalbasis
    \end{itemize}
\end{sectionbox}
...
\begin{sectionbox}
    ...
    \begin{itemize}
        \item $f$ ist diagonalisierbar, falls $V$ eine Basis aus Eigenvektoren besitzt
        \item $D_f \in K^{n\times n}$ ist diagonalisierbar, falls $B^{-1}D_fB$ Diagonalgestalt hat ($B$: Basiswechselmatrix, $D_f$: Darstellungsmatrix)
        \item $V$ kann als direkte Summe aller Eigenräume geschrieben werden: $\dim E_{\lambda_1} + ... + \dim E_{\lambda_k}=\dim V$. Die Basis aus Eigenvektoren bildet sich aus der Vereinigung der Basen der Eigenräume.\\
              \textit{Hat man z.B. zwei Mal den gleichen Eigenwert $\mu$, so ist $A$ nur diagonalisierbar, falls $\dim E_\mu=2$ ist. Es also zwei Eigenvektoren zu dem Eigenwert $\mu$ gibt}
        \item Jede Selbstadjungierte ($f^*=f$) ist diagonalisierbar mit einer Orthonormalbasis aus Eigenvektoren
    \end{itemize}
\end{sectionbox}

\begin{sectionbox}
    \subsubsection{Spur $\tr A$}
    Summe der Diagonaleinträge einer Matrix. Immer gleich der Summe der Eigenwerte
\end{sectionbox}
